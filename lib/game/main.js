  ...
  drawFog: function() {
    for (var x = 0; x < this.mapTileSize.x; x++) {
      ...
      for (var y = 0; y < this.mapTileSize.y; y++) {
        ...
        if(this.player.fov.viewedTiles[key] || y === 29) {
          // Draw strip and reset
          ig.system.context.fillStyle = "rgba(0, 0, 0, 1)";
          ig.system.context.fillRect(newX, fillY, 16, tiles * 16);
          tiles = 0;
          fillX = -1;
          fillY = -1;
        } else {
    ...
    for (var x = 0; x < this.mapTileSize.x; x++) {
      for (var y = 0; y < this.mapTileSize.y; y++) {
        ...
        if(this.player.fov.viewedTiles[key]) {
          // ig.system.context.fillStyle = "rgba(0, 0, 0, 1)";
          // ig.system.context.fillRect(newX, newY, 16, strip);
          strip = 0;
        } else {
          ig.system.context.fillStyle = "rgba(0, 0, 0, 1)";
          ig.system.context.fillRect(x, y, 16, 16);
          strip += 16;
        }
      }
    }
