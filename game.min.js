...
// lib/plugins/gui.js
ig.baked=true;ig.module('plugins.gui').requires('impact.game').defines(function(){ig.gui={show:true,initialized:false,cursor:{pos:{x:0,y:0},offset:{x:0,y:0},image:null,set:function(image){this.image=image;},clear:function(){this.image=null;},draw:function(){if(this.image!=null)
this.image.draw(this.pos.x-this.offset.x,this.pos.y-this.offset.y);}},elements:[],element:{action:function(action,name){var collection=[];for(var i=0;i<ig.gui.elements.length;i++){if(action=='getByName'&&ig.gui.elements[i].name==name)
...
ig.gui.elements[i].disable=false;if(action=='enable'&&ig.gui.elements[i].name==name)
...
if(collection.length){if(collection.length==1)collection=collection[0];return collection;}},add:function(element){if(element.show==undefined)element.show=true;if(element.disabled==undefined)element.disabled=false;if(element.active==undefined)element.active=false;ig.gui.elements.push(element);},draw:function(){for(var i=0;i<ig.gui.elements.length;i++){var element=ig.gui.elements[i],state='normal';if(!element.show)continue;if(ig.gui.cursor.pos.x>=element.pos.x&&ig.gui.cursor.pos.x<=element.pos.x+element.size.x&&ig.gui.cursor.pos.y>=element.pos.y&&ig.gui.cursor.pos.y<=element.pos.y+element.size.y&&!element.disabled){state='hover';}
if(state=='hover'&&(ig.input.state('mouse1')||ig.input.pressed('mouse1'))){state='active';if(ig.input.state('mouse1')&&typeof ig.gui.elements[i].mouseDown=='function')
ig.gui.elements[i].mouseDown.call(element);if(ig.input.pressed('mouse1')){if(element.toggle)
...
state='normal';if(element.alpha!=undefined)
ig.system.context.globalAlpha=element.alpha;var image=ig.gui.elements[i].state[state];if(isNaN(image.tile)||isNaN(image.tileSize)){image.image.draw(element.pos.x,element.pos.y);}else{image.image.drawTile(element.pos.x,element.pos.y,image.tile,image.tileSize);}
ig.system.context.globalAlpha=1;}}},draw:function(){this.element.draw();this.cursor.pos={x:ig.input.mouse.x,y:ig.input.mouse.y}
this.cursor.draw();}}
ig.Game.inject({update:function(){this.parent();if(ig.gui.initialized)return;else ig.gui.initialized=true;ig.input.bind(ig.KEY.MOUSE1,'mouse1');}});});
...
ig.baked=true;ig.module('game.entities.character').requires('impact.entity','plugins.dice','plugins.attack').defines(function(){EntityCharacter=ig.Entity.extend({baseAC:10,baseLevel:0,experience:0,baseExperience:0,health:10,maxHealth:10,damageDice:new Dice(2,1),init:function(x,y,settings){this.parent(x,y,settings);},act:function(){},AC:function(){return this.baseAC;},level:function(){return this.baseLevel;},fight:function(other){this.attack(other);other.attack(this);if(!other.alive()){this.experience+=other.baseExperience;}},attack:function(other){if(this.alive()&&other.alive()){var attack=new Attack(this,other);if(attack.hit){ig.game.log.push(this.name+" hits "+other.name+" for "+attack.damage);other.receiveDamage(attack.damage,this);}else{ig.game.log.push(this.name+" misses!");}}},attackDamage:function(){return this.damageDice.roll();},receiveDamage:function(amount,from){this.parent(amount,from);if(!this.alive()){ig.game.log.push(this.name+" dies!");}},alive:function(){return this.health>0;}});});

// lib/plugins/fov.js
ig.baked=true;ig.module('plugins.fov').requires('impact.impact').defines(function(){FOV=ig.Class.extend({entity:null,algorithm:null,viewedTiles:{},visibilityRadius:5,init:function(entity){this.entity=entity;this.algorithm=new ROT.FOV.PreciseShadowcasting(_.bind(this._inputCallback,this));},compute:function(){var posX=Math.floor(this.entity.pos.x/8);var posY=Math.floor(this.entity.pos.y/8);this.algorithm.compute(posX,posY,this.visibilityRadius,_.bind(this._outputCallback,this));},_inputCallback:function(x,y){var row=ig.game.collisionTiles[y];var tile=null;if(row){tile=row[x];}
...
ig.baked=true;ig.module('game.entities.archer').requires('game.entities.character','plugins.fov').defines(function(){EntityArcher=EntityCharacter.extend({collides:ig.Entity.COLLIDES.ACTIVE,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.B,size:{x:8,y:8},health:20,maxHealth:20,gridSpeed:{x:50,y:50},name:"Archer",timePressed:0,repeatWait:3,fov:null,animSheet:new ig.AnimationSheet('media/archer.png',8,8),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('right',0.1,[0]);this.addAnim('down',0.1,[1]);this.addAnim('left',0.1,[2]);this.addAnim('up',0.1,[3]);this.currentAnim=this.anims.right;this.fov=new FOV(this);},act:function(){ig.game.engine.lock();this.fov.compute();},update:function(){this.parent();var moved=false;var dirX=0;var dirY=0;var newX=0;var newY=0;if(ig.input.pressed('left')){dirX=-8;this.currentAnim=this.anims.left;this.timePressed=-25;}
else if(ig.input.pressed('right')){dirX=8;this.currentAnim=this.anims.right;this.timePressed=-25;}
else if(ig.input.pressed('up')){dirY=-8;this.currentAnim=this.anims.up;this.timePressed=-25;}
else if(ig.input.pressed('down')){dirY=8;this.currentAnim=this.anims.down;this.timePressed=-25;}
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','plugins.gui','game.entities.archer','game.entities.stairs','game.entities.skeleton').defines(function(){LeftAxe=ig.Game.extend({font:new ig.Font('media/04b03.font.png'),hud:new ig.Image('media/hud.png'),hp:new ig.Image('media/hp.png'),log:[],dungeon:null,fov:null,visitedTiles:{},freeTiles:[],map:null,mapTiles:[],collisionTiles:[],mapTileSize:{x:45,y:30},tileCount:0,console:null,engine:null,player:null,scheduler:new ROT.Scheduler.Simple(),init:function(){ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.H,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.L,'right');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.K,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.J,'down');ig.input.bind(ig.KEY.SPACE,'shoot');this._generateMap();this.collision_map=this.mapTiles;this.engine=new ROT.Engine(this.scheduler);this.engine.start();},_generateMap:function(){for(var y=0;y<this.mapTileSize.y;y++){var row1=[];var row2=[];for(var x=0;x<this.mapTileSize.x;x++){row1.push(0);row2.push(1);}
this.mapTiles[y]=row1;this.collisionTiles[y]=row2;}
var digger=new ROT.Map.Digger(this.mapTileSize.x,this.mapTileSize.y,{dugPercentage:0.8,corridorLength:[1,3]});var digCallback=function(x,y,value){this.collisionTiles[y][x]=value;if(value===0){this.freeTiles.push({x:x,y:y});this.mapTiles[y][x]=1;}else{this.mapTiles[y][x]=2;}};digger.create(digCallback.bind(this));this._drawMap();this._createPlayer();this._createEntities();},_drawMap:function(){var map=new ig.BackgroundMap(8,this.mapTiles,'media/dungeon.png');map.preRender=true;map.chunkSize=640;this.backgroundMaps.push(map);this.collisionMap=new ig.CollisionMap(8,this.collisionTiles);},_createPlayer:function(){var index=Math.floor(ROT.RNG.getUniform()*this.freeTiles.length);var pos=this.freeTiles.splice(index,1)[0];this.player=ig.game.spawnEntity(EntityArcher,pos.x*8,pos.y*8);this.scheduler.add(this.player,true);},_createEntities:function(){var index,pos,entity;for(var i=0;i<10;i++){index=Math.floor(ROT.RNG.getUniform()*this.freeTiles.length);pos=this.freeTiles.splice(index,1)[0];entity=ig.game.spawnEntity(EntitySkeleton,pos.x*8,pos.y*8);this.scheduler.add(entity,true);}
index=Math.floor(ROT.RNG.getUniform()*this.freeTiles.length);pos=this.freeTiles.splice(index,1)[0];entity=ig.game.spawnEntity(EntityStairs,pos.x*8,pos.y*8);},passableTile:function(x,y){var row=this.collisionTiles[y];var tile=null;if(row){tile=row[x];}
if(typeof tile=="undefined"){return false;}else{return tile===0;}},update:function(){this.parent();},draw:function(){this.parent();if(this.player){this.drawMenu();}
this.drawLog();this.drawFog();if(ig.input.pressed('inventory')){alert('toggle inventory');}},drawMenu:function(){this.hud.draw(3,245);var fullBar=82;var currentHealth=Math.floor((this.player.health/this.player.maxHealth)*fullBar);ig.system.context.fillStyle="red";ig.system.context.fillRect(59,506,currentHealth,8);if(this.player.experience>0){var currentXP=Math.floor((this.player.experience/100)*fullBar);ig.system.context.fillStyle="yellow";ig.system.context.fillRect(59,522,currentXP,8);}
this.font.draw(this.player.AC(),85,257);},drawFog:function(){for(var x=0;x<this.mapTileSize.x;x++){for(var y=0;y<this.mapTileSize.y;y++){key=x+','+y;if(this.player.fov.viewedTiles[key]){}else{newX=Math.floor(x*16);newY=Math.floor(y*16);ig.system.context.fillStyle="rgba(0, 0, 0, 1)";ig.system.context.fillRect(newX,newY,16,16);}}}},drawLog:function(){var x=120;var y=245;var entriesToShow=this.log.length>=2?2:this.log.length;for(var i=entriesToShow;i>0;i--){var entry=this.log[this.log.length-i];if(entry){this.font.draw(entry,x,y);y+=10;}}}});ig.main('#canvas',LeftAxe,60,360,310,2);});