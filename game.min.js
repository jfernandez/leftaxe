...
else{window.ig.setAnimation=function(callback,element){return window.setInterval(callback,1000/15);};window.ig.clearAnimation=function(id){window.clearInterval(id);};}
...
// lib/plugins/fog.js
ig.baked=true;ig.module('plugins.fog').requires('impact.impact').defines(function(){ig.Fog=ig.Class.extend({fillStyle:'rgba(0, 0, 0, 1)',init:function(mapWidth,mapHeight,tileSize){this.mapWidth=mapWidth;this.mapHeight=mapHeight;this.tileSize=tileSize;},draw:function(viewedTileCallback){var fogColumns=[];var column=null;for(var x=0;x<this.mapWidth;x++){for(var y=0;y<this.mapHeight;y++){if(!column){column={x:x,y:y,tiles:0};}
if(viewedTileCallback(x,y)){if(column.tiles>0){fogColumns.push(column);}
column=null;}else if((y+1)===this.mapHeight){column.tiles++;fogColumns.push(column);column=null;}else{column.tiles++;}}}
for(var i=0;i<fogColumns.length;i++){var col=fogColumns[i];this._drawColumn(col.x,col.y,col.tiles);}},_drawColumn:function(x,y,tiles){ig.system.context.fillStyle=this.fillStyle;ig.system.context.fillRect(this._realSize(x),this._realSize(y),this._realSize(1),this._realSize(tiles));},_realSize:function(number){return number*this.tileSize*ig.system.scale;}});});

// lib/game/inventory.js
ig.baked=true;ig.module('game.inventory').requires('impact.font').defines(function(){Inventory=ig.Class.extend({font:new ig.Font('media/04b03.font.png'),stats:new ig.Image('media/stats.png'),tooltip:new ig.Image('media/tooltip.png'),clearColor:'rgba(0, 0, 0, 1)',init:function(){this.selectionIndex=0;this.items=[];},addItem:function(item){this.items.push(item);},dropItem:function(){var item=this.selectedItem();this.items.splice(this.selectionIndex,1);if(this.selectionIndex>0){this.selectionIndex--;}
this.adjacentSpawn(item);},adjacentSpawn:function(item){var x=Math.floor(ig.game.player.pos.x/8);var y=Math.floor(ig.game.player.pos.y/8);if(ig.game.passableTile(x,y-1)){y--;}else if(ig.game.passableTile(x,y+1)){y++;}else if(ig.game.passableTile(x-1,y)){x--;}else if(ig.game.passableTile(x+1,y)){x++;}
ig.game.spawnEntity(item.entityClass,Math.floor(x*8),Math.floor(y*8),{name:item.name});},update:function(){if(ig.input.pressed('up')&&this.selectionIndex!==0){this.selectionIndex--;}
if(ig.input.pressed('down')&&this.selectionIndex!==(this.items.length-1)){this.selectionIndex++;}
if(ig.input.pressed('enter')){this.selectItem();}
if(ig.input.pressed('drop')){this.dropItem();}},draw:function(){if(this.clearColor){ig.system.context.fillStyle=this.clearColor;ig.system.context.fillRect(0,0,ig.system.realWidth,ig.system.realHeight);}
this.font.draw("[Enter] to equip, or (d)rop",10,80,ig.Font.ALIGN.LEFT);var y=20;for(var i=0;i<this.items.length;i++){var item=this.items[i];var description=item.name;if(ig.game.player.armor&&item.name===ig.game.player.armor.name){description+=" (Worn)";}
this.font.draw(description,(ig.system.width/2)-30,y,ig.Font.ALIGN.LEFT);if(this.selectionIndex==i){this.font.draw('=>',(ig.system.width/2)-37,y,ig.Font.ALIGN.CENTER);}
y+=10;}
this.drawStats();this.drawTooltip();},drawTooltip:function(){var item=this.selectedItem();this.tooltip.draw(10,10);this.font.draw(item.name,40,23);this.font.draw(item.description(),40,45);item.icons.drawTile(18,18,item.spriteIndex,item.largeSpriteSize);},drawStats:function(){this.stats.draw(3,245);this.font.draw(ig.game.player.armorClass(),25,272);},selectedItem:function(){return this.items[this.selectionIndex];},selectItem:function(){ig.game.player.armor=this.selectedItem();}});});

// lib/game/entities/item.js
ig.baked=true;ig.module('game.entities.item').requires('impact.entity').defines(function(){EntityItem=ig.Entity.extend({spriteIndex:0,spriteSize:8,largeSpriteSize:16,size:{x:8,y:8},checkAgainst:ig.Entity.TYPE.A,init:function(x,y,settings){var item=EntityItem.ITEMS[settings.name];this.parent(x,y,ig.merge(settings,item));this.addAnim('idle',0.1,[this.spriteIndex]);this.currentAnim=this.anims.idle;},description:function(){return this.name;},armorClass:function(){return 0;},check:function(other){ig.game.log.push("You pick up a "+this.name);ig.game.inventory.addItem(this);this.kill();}});EntityItem.ITEMS={};});

// lib/game/entities/weapon.js
ig.baked=true;ig.module('game.entities.weapon').requires('game.entities.item').defines(function(){EntityWeapon=EntityItem.extend({entityClass:"EntityWeapon",animSheet:new ig.AnimationSheet('media/weapon.png',8,8),icons:new ig.Image('media/weapon-large.png')});EntityWeapon.ITEMS={"Dagger":{spriteIndex:0,minLevel:1,maxLevel:10,weight:100},"Orcish Dagger":{spriteIndex:0,minLevel:1,maxLevel:10,weight:100},"Silver Dagger":{spriteIndex:0,minLevel:1,maxLevel:10,weight:100},"Elven Dagger":{spriteIndex:0,minLevel:1,maxLevel:10,weight:100},"Long Sword":{spriteIndex:1,minLevel:1,maxLevel:10,weight:100},"Excalibur":{spriteIndex:2,minLevel:1,maxLevel:10,weight:100}};ig.merge(EntityItem.ITEMS,EntityWeapon.ITEMS);});

// lib/game/entities/armor.js
ig.baked=true;ig.module('game.entities.armor').requires('game.entities.item').defines(function(){EntityArmor=EntityItem.extend({entityClass:"EntityArmor",animSheet:new ig.AnimationSheet('media/armor.png',8,8),icons:new ig.Image('media/armor-large.png'),armorClass:function(){return this.baseArmorClass;},description:function(){return"Armor Class: "+this.armorClass();}});EntityArmor.ITEMS={"Leather Jacket":{baseArmorClass:1,spriteIndex:0,minLevel:1,maxLevel:10,weight:100},"Leather Armor":{baseArmorClass:2,spriteIndex:0,minLevel:1,maxLevel:10,weight:30},"Orcish Ring Mail":{baseArmorClass:2,spriteIndex:1,minLevel:1,maxLevel:10,weight:20},"Studded Leather Armor":{baseArmorClass:3,spriteIndex:0,minLevel:1,maxLevel:10,weight:20},"Ring Mail":{baseArmorClass:3,spriteIndex:2,minLevel:1,maxLevel:10,weight:10},"Dragon Scales":{baseArmorClass:3,spriteIndex:2,minLevel:1,maxLevel:10,weight:5},"Orcish Chain Mail":{baseArmorClass:4,spriteIndex:3,minLevel:1,maxLevel:10,weight:5},"Scale Mail":{baseArmorClass:4,spriteIndex:3,minLevel:1,maxLevel:10,weight:5},"Elven Mithril-Coat":{baseArmorClass:5,spriteIndex:4,minLevel:1,maxLevel:10,weight:5},"Chain Mail":{baseArmorClass:5,spriteIndex:4,minLevel:1,maxLevel:10,weight:5}};EntityArmor.sumOfWeights=function(){var sum=0;for(var key in EntityArmor.ITEMS){sum+=EntityArmor.ITEMS[key].weight;}
return sum;};ig.merge(EntityItem.ITEMS,EntityArmor.ITEMS);});

// lib/game/itemizer.js
ig.baked=true;ig.module('game.itemizer').requires('impact.impact','game.entities.weapon','game.entities.armor').defines(function(){Itemizer=ig.Class.extend({armorRatio:0.5,weaponRatio:0.5,maximumItems:8,armor:[],weapons:[],init:function(){var armorCount=this.maximumItems*this.armorRatio;var weaponCount=this.maximumItems*this.weaponRatio;var i;for(i=1;i<=armorCount;i++){this.armor.push(this.randomItem(EntityArmor.ITEMS));}
for(i=1;i<=weaponCount;i++){this.weapons.push(this.randomItem(EntityWeapon.ITEMS));}},randomItem:function(items){var totalWeight=_.reduce(items,function(sum,item){return sum+item.weight;},0);var cumulativeWeight=0;var random=Math.floor(Math.random()*totalWeight);var key,item;for(key in items){item=items[key];cumulativeWeight+=item.weight;if(random<cumulativeWeight)
return key;}}});});

// lib/game/entities/door.js
ig.baked=true;ig.module('game.entities.door').requires('impact.entity').defines(function(){EntityDoor=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/dungeon.png',8,8),size:{x:8,y:8},checkAgainst:ig.Entity.TYPE.A,closed:true,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('closed',0.1,[2]);this.addAnim('open',0.1,[3]);this.currentAnim=this.anims.closed;},open:function(){var key;key=(this.pos.x/8)+','+(this.pos.y/8);this.closed=false;this.currentAnim=this.anims.open;ig.game.dungeon.doors[key]=false;ig.game.player.fov.compute();},check:function(other){if(this.closed){this.open();}}});});

// lib/game/dungeon.js
ig.baked=true;ig.module('game.dungeon').requires('impact.impact','game.itemizer','game.entities.door').defines(function(){Dungeon=ig.Class.extend({scheduler:new ROT.Scheduler.Simple(),backgroundTiles:[],collisionTiles:[],freeTiles:[],entities:[],doors:{},visitedTiles:{},armorDropRate:0.5,weaponDropRate:0.5,maximumItems:10,init:function(width,height){this.width=width;this.height=height;this.initializeMaps();this.generateMap();this.generateEntities();},items:function(){var numArmor=Math.floor(this.maximumItems*this.armorDropRate);var numWeapon=Math.floor(this.maximumItems*this.weaponDropRate);},load:function(){var map=new ig.BackgroundMap(8,this.backgroundTiles,'media/dungeon.png');map.preRender=true;map.chunkSize=640;ig.game.backgroundMaps.push(map);ig.game.collisionMap=new ig.CollisionMap(8,this.collisionTiles);},initializeMaps:function(){for(var y=0;y<this.height;y++){var row1=[],row2=[];for(var x=0;x<this.width;x++){row1.push(0);row2.push(1);}
this.backgroundTiles[y]=row1;this.collisionTiles[y]=row2;}},generateMap:function(){var digger=new ROT.Map.Digger(this.width,this.height,{dugPercentage:0.8,corridorLength:[1,3]});var digCallback=function(x,y,value){this.collisionTiles[y][x]=value;if(value===0){this.freeTiles.push({x:x,y:y});this.backgroundTiles[y][x]=1;}else{this.backgroundTiles[y][x]=2;}};digger.create(digCallback.bind(this));var doorCallback=function(x,y){this.doors[x+','+y]=true;};var rooms=digger.getRooms();for(var i=0;i<rooms.length;i++){var room=rooms[i];room.getDoors(doorCallback.bind(this));}},generateEntities:function(){var index,pos,entity,i,door,itemName;for(i=0;i<10;i++){index=Math.floor(ROT.RNG.getUniform()*this.freeTiles.length);pos=this.freeTiles.splice(index,1)[0];entity=ig.game.spawnEntity(EntitySkeleton,pos.x*8,pos.y*8);this.scheduler.add(entity,true);}
var itemizer=new Itemizer();for(i=0;i<itemizer.weapons.length;i++){itemName=itemizer.weapons[i];index=Math.floor(ROT.RNG.getUniform()*this.freeTiles.length);pos=this.freeTiles.splice(index,1)[0];ig.game.spawnEntity(EntityWeapon,pos.x*8,pos.y*8,{name:itemName});}
for(i=0;i<itemizer.armor.length;i++){itemName=itemizer.armor[i];index=Math.floor(ROT.RNG.getUniform()*this.freeTiles.length);pos=this.freeTiles.splice(index,1)[0];ig.game.spawnEntity(EntityArmor,pos.x*8,pos.y*8,{name:itemName});}
for(i=0;i<5;i++){index=Math.floor(ROT.RNG.getUniform()*this.freeTiles.length);pos=this.freeTiles.splice(index,1)[0];entity=ig.game.spawnEntity(EntityPotion,pos.x*8,pos.y*8);}
index=Math.floor(ROT.RNG.getUniform()*this.freeTiles.length);pos=this.freeTiles.splice(index,1)[0];entity=ig.game.spawnEntity(EntityStairs,pos.x*8,pos.y*8);_.each(this.doors,function(value,key){var x=key.split(',')[0];var y=key.split(',')[1];ig.game.spawnEntity(EntityDoor,x*8,y*8);});},spawnPlayer:function(player){var index=Math.floor(ROT.RNG.getUniform()*this.freeTiles.length);var pos=this.freeTiles.splice(index,1)[0];var stairs=ig.game.spawnEntity(EntityStairs,pos.x*8,pos.y*8);stairs.currentAnim=stairs.anims.up;player.pos.x=pos.x*8;player.pos.y=pos.y*8;ig.game.entities.push(player);this.scheduler.add(player,true);}});});

// lib/game/dice.js
ig.baked=true;ig.module('game.dice').requires('impact.impact').defines(function(){Dice=ig.Class.extend({sides:null,quantity:null,init:function(sides,quantity){this.sides=sides;this.quantity=quantity||1;},roll:function(){var amount=0;for(var i=1;i<=this.quantity;i++){amount+=this._randomNumber(this.sides);}
return amount;},_randomNumber:function(max){return Math.floor((Math.random()*max)+1);}});});

// lib/game/attack.js
ig.baked=true;ig.module('game.attack').requires('impact.impact','game.dice').defines(function(){Attack=ig.Class.extend({attacker:null,defender:null,hit:false,damage:0,attackRoll:0,attackDice:new Dice(20),init:function(attacker,defender){this.attacker=attacker;this.defender=defender;this.attackRoll=this.attackDice.roll();if(this.attackRoll<=this.targetNumber()){this.hit=true;this.damage=attacker.attackDamage();}},targetNumber:function(){return 10+this.defender.AC()+this.attacker.level();}});});

// lib/game/entities/creature.js
ig.baked=true;ig.module('game.entities.creature').requires('impact.entity','game.dice','game.attack').defines(function(){EntityCreature=ig.Entity.extend({armor:null,weapon:null,shield:null,baseAC:10,baseLevel:0,experience:0,baseExperience:0,health:10,maxHealth:10,damageDice:new Dice(2,1),init:function(x,y,settings){this.parent(x,y,settings);},act:function(){},AC:function(){return this.baseAC;},armorClass:function(){return this.baseAC-this._equipmentArmorClass();},_equipmentArmorClass:function(){var amount=0;if(this.armor)amount+=this.armor.armorClass();if(this.shield)amount+=this.shield.armorClass();return amount;},level:function(){return this.baseLevel;},heal:function(amount,from){this.health+=amount;if(this.health>this.maxHealth){this.health=this.maxHealth;}},fight:function(other){this.attack(other);other.attack(this);if(!other.alive()){this.experience+=other.baseExperience;}},attack:function(other){if(this.alive()&&other.alive()){var attack=new Attack(this,other);if(attack.hit){ig.game.log.push(this.name+" hits "+other.name+" for "+attack.damage);other.receiveDamage(attack.damage,this);}else{ig.game.log.push(this.name+" misses!");}}},attackDamage:function(){return this.damageDice.roll();},receiveDamage:function(amount,from){this.parent(amount,from);if(!this.alive()){ig.game.log.push(this.name+" dies!");}},alive:function(){return this.health>0;}});});

// lib/plugins/fov.js
ig.baked=true;ig.module('plugins.fov').requires('impact.impact').defines(function(){FOV=ig.Class.extend({entity:null,algorithm:null,viewedTiles:{},visibilityRadius:5,init:function(entity,visibilityRadius){this.entity=entity;this.visibilityRadius=visibilityRadius;this.algorithm=new ROT.FOV.PreciseShadowcasting(_.bind(this._inputCallback,this));},compute:function(){var posX=Math.floor(this.entity.pos.x/8);var posY=Math.floor(this.entity.pos.y/8);this.algorithm.compute(posX,posY,this.visibilityRadius,_.bind(this._outputCallback,this));},_inputCallback:function(x,y){var door=ig.game.dungeon.doors[x+','+y];if(door){return false;}
var row=ig.game.dungeon.collisionTiles[y];var tile=null;if(row){tile=row[x];}
if(typeof tile==="undefined"){return false;}else{return tile===0;}},_outputCallback:function(x,y,r,visibility){tile=x+','+y;this.viewedTiles[tile]=true;}});});

// lib/game/entities/player.js
ig.baked=true;ig.module('game.entities.player').requires('game.entities.creature','game.entities.armor','game.entities.weapon','plugins.fov').defines(function(){EntityPlayer=EntityCreature.extend({collides:ig.Entity.COLLIDES.ACTIVE,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.B,size:{x:8,y:8},health:20,maxHealth:20,gridSpeed:{x:50,y:50},name:"Player",timePressed:0,repeatWait:0,fov:null,animSheet:new ig.AnimationSheet('media/player.png',8,8),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('right',0.1,[0]);this.addAnim('down',0.1,[1]);this.addAnim('left',0.1,[2]);this.addAnim('up',0.1,[3]);this.currentAnim=this.anims.right;this.fov=new FOV(this,5);var armor1=new EntityArmor(0,0,{name:'Leather Armor'});var armor2=new EntityArmor(0,0,{name:'Ring Mail'});var armor3=new EntityArmor(0,0,{name:'Elven Mithril-Coat'});var weapon=new EntityWeapon(0,0,{name:'Long Sword'});ig.game.inventory.addItem(armor1);ig.game.inventory.addItem(armor2);ig.game.inventory.addItem(armor3);ig.game.inventory.addItem(weapon);this.armor=armor1;},act:function(){ig.game.engine.lock();this.fov.compute();},update:function(){this.parent();if(ig.game.state=='inventory')return;var moved=false;var dirX=0;var dirY=0;var newX=0;var newY=0;if(ig.input.pressed('left')){dirX=-8;this.currentAnim=this.anims.left;this.timePressed=-5;}
else if(ig.input.pressed('right')){dirX=8;this.currentAnim=this.anims.right;this.timePressed=-5;}
else if(ig.input.pressed('up')){dirY=-8;this.currentAnim=this.anims.up;this.timePressed=-5;}
else if(ig.input.pressed('down')){dirY=8;this.currentAnim=this.anims.down;this.timePressed=-5;}
if(ig.input.state('left')){this.timePressed++;if(this.timePressed>this.repeatWait){dirX=-8;this.currentAnim=this.anims.left;this.timePressed=0;}}
else if(ig.input.state('right')){this.timePressed++;if(this.timePressed>this.repeatWait){dirX=8;this.currentAnim=this.anims.right;this.timePressed=0;}}
else if(ig.input.state('up')){this.timePressed++;if(this.timePressed>this.repeatWait){dirY=-8;this.currentAnim=this.anims.up;this.timePressed=0;}}
else if(ig.input.state('down')){this.timePressed++;if(this.timePressed>this.repeatWait){dirY=8;this.currentAnim=this.anims.down;this.timePressed=0;}}
moved=dirX!==0||dirY!==0;if(moved){newX=this.pos.x+dirX;newY=this.pos.y+dirY;if(ig.game.passableTile(Math.floor(newX/8),Math.floor(newY/8))){this.pos.x=newX;this.pos.y=newY;}
ig.game.engine.unlock();}},check:function(other){this.parent(other);this.fight(other);}});});

// lib/game/entities/stairs.js
ig.baked=true;ig.module('game.entities.stairs').requires('impact.entity').defines(function(){EntityStairs=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/environment.png',8,8),size:{x:8,y:8},checkAgainst:ig.Entity.TYPE.A,messageTimer:null,font:new ig.Font('media/04b03.font.png'),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('down',0.1,[24]);this.addAnim('up',0.1,[23]);this.currentAnim=this.anims.down;},check:function(other){if(this.currentAnim===this.anims.down){ig.game.loadDungeon();}}});});

// lib/game/entities/skeleton.js
ig.baked=true;ig.module('game.entities.skeleton').requires('game.entities.creature').defines(function(){EntitySkeleton=EntityCreature.extend({collides:ig.Entity.COLLIDES.FIXED,type:ig.Entity.TYPE.B,checkAgainst:ig.Entity.TYPE.A,size:{x:8,y:8},animSheet:new ig.AnimationSheet('media/undead.png',8,8),name:"Skeleton",health:3,baseExperience:7,baseAC:9,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.1,[0]);this.currentAnim=this.anims.idle;},act:function(){ig.game.engine.lock();var targetX=Math.floor(ig.game.player.pos.x/8);var targetY=Math.floor(ig.game.player.pos.y/8);var currentX=Math.floor(this.pos.x/8);var currentY=Math.floor(this.pos.y/8);var distance=Math.sqrt(Math.pow(targetX-currentX,2)+Math.pow(targetY-currentY,2));if(distance<4){var astar=new ROT.Path.AStar(targetX,targetY,ig.game.passableTile.bind(ig.game),{topology:4});var path=[];var pathCallback=function(x,y){path.push([x,y]);};astar.compute(currentX,currentY,pathCallback);path.shift();if(path.length>0){var newX=Math.floor(path[0][0]*8);var newY=Math.floor(path[0][1]*8);this.pos.x=newX;this.pos.y=newY;}}
ig.game.engine.unlock();}});});

// lib/game/entities/potion.js
ig.baked=true;ig.module('game.entities.potion').requires('impact.entity').defines(function(){EntityPotion=ig.Entity.extend({animSheet:new ig.AnimationSheet('media/objects.png',8,8),size:{x:8,y:8},checkAgainst:ig.Entity.TYPE.A,name:"Health Potion",init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.1,[12]);this.currentAnim=this.anims.idle;},check:function(other){other.heal(4,this);ig.game.log.push(other.name+" is healed by a "+this.name);this.kill();}});});

// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','plugins.fog','game.inventory','game.dungeon','game.entities.player','game.entities.stairs','game.entities.skeleton','game.entities.potion').defines(function(){LeftAxe=ig.Game.extend({font:new ig.Font('media/04b03.font.png'),hud:new ig.Image('media/hud.png'),hp:new ig.Image('media/hp.png'),log:[],dungeons:[],mapTiles:[],collisionTiles:[],mapWidth:45,mapHeight:30,tileCount:0,tileSize:8,console:null,state:'game',init:function(){this.bindKeys();this.inventory=new Inventory();this.player=new EntityPlayer();this.fog=new ig.Fog(this.mapWidth,this.mapHeight,this.tileSize);this.scheduler=new ROT.Scheduler.Simple();this.dungeonLevel=0;this.loadDungeon();},loadDungeon:function(){this.entities=[];this.namedEntities={};this.collisionMap=ig.CollisionMap.staticNoCollision;this.player.fov.viewedTiles={};this.dungeon=new Dungeon(this.mapWidth,this.mapHeight);this.dungeon.load();this.dungeon.spawnPlayer(this.player);this.dungeons.push(this.dungeon);this.dungeons++;this.engine=new ROT.Engine(this.dungeon.scheduler);this.engine.start();},update:function(){this.parent();if(ig.input.pressed('inventory')){this.toggleState();}
if(this.state==='inventory'){this.inventory.update();}},toggleState:function(){if(this.state==='game'){this.state='inventory';}else{this.state='game';}},draw:function(){if(this.state==='inventory'){this.inventory.draw();return;}
this.parent();if(this.player){this.drawMenu();}
this.drawLog();this.fog.draw(this.viewedTile.bind(this));},viewedTile:function(x,y){var key=x+','+y;return this.player.fov.viewedTiles[key];},passableTile:function(x,y){var row=this.dungeon.collisionTiles[y];var tile;if(row){tile=row[x];}
if(typeof tile==='undefined'){return false;}else{return tile===0;}},drawMenu:function(){this.hud.draw(3,245);var fullBar=82;var currentHealth=Math.floor((this.player.health/this.player.maxHealth)*fullBar);ig.system.context.fillStyle="red";ig.system.context.fillRect(59,506,currentHealth,8);if(this.player.experience>0){var currentXP=Math.floor((this.player.experience/100)*fullBar);ig.system.context.fillStyle="yellow";ig.system.context.fillRect(59,522,currentXP,8);}
if(this.player&&this.player.armor){var armor=this.player.armor;armor.icons.drawTile(9,278,armor.spriteIndex,armor.largeSpriteSize);}
this.font.draw(this.player.armorClass(),85,257);this.font.draw("[Space] for inventory",130,280);},drawLog:function(){var x=120;var y=245;var entriesToShow=this.log.length>=2?2:this.log.length;for(var i=entriesToShow;i>0;i--){var entry=this.log[this.log.length-i];if(entry){this.font.draw(entry,x,y);y+=10;}}},bindKeys:function(){ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.H,'left');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.L,'right');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.K,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.J,'down');ig.input.bind(ig.KEY.SPACE,'inventory');ig.input.bind(ig.KEY.ENTER,'enter');ig.input.bind(ig.KEY.D,'drop');}});ig.main('#canvas',LeftAxe,30,360,310,2);});